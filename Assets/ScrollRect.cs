                ...
                if (!this.m_Dragging && ((offset != Vector2.zero) || (this.m_Velocity != Vector2.zero)))
                {
                    ...
                    for (int i = 0; i < 2; i++)
                    {
                        ...
                        else if (this.m_Inertia)
                        {
                            ...
                            //int num5;
                            //int num6;
                            //(vectorRef = (Vector2) & this.m_Velocity)[num5 = i] = vectorRef[num5] * Mathf.Pow(this.m_DecelerationRate, unscaledDeltaTime);
                            //if (Mathf.Abs(this.m_Velocity[i]) < 1f)
                            //{
                            //    this.m_Velocity[i] = 0f;
                            //}
                            //(vectorRef = (Vector2) & anchoredPosition)[num6 = i] = vectorRef[num6] + (this.m_Velocity[i] * unscaledDeltaTime);
                        }
                        else
                        {
                            this.m_Velocity[i] = 0f;
